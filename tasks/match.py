...
def in_params():
        ...
        for file in os.listdir(in_path):
            ...
            if isfile(f):
                if f.endswith('.fits'):
                    fits_list.append(f)
    if not fits_list:
        print("{}\n No .fits files found in 'match_fldr' folder. Exit.".format(
            in_path))
        sys.exit()

    in_path = join(pars['mypath'].replace('tasks', 'input'))
    if type(pars['ref_field']) is list:
        # A Landolt field was set. We are matching standard frames.
        pars['landolt_fld'], pars['ref_id_std'] = pars['ref_field'][0]
    else:
        pars['landolt_fld'], pars['ref_id_std'] = pars['ref_field'],\
            pars['ref_field']
    ...
    # Name of image used as reference.
    pars['f_name'] = pars['ref_id_std'].replace(pars['mypath'].replace(
        'tasks', 'input'), '')

    import pdb; pdb.set_trace()  # breakpoint a9461eaa //
...
def main():
    ...
    print("Reference frame: {}".format(pars['f_name']))
    ...
    if pars['do_plots_C'] == 'y':
        ...
        make_plot(
            pars['f_name'], out_plot_file, xy_obs, obs_mag, std_tr_match,
