...
begin
      ...
      bool sigma_warning, apert_warning, nopsfstar, pstselect_while
      bool thresh_change, tvmark, pstauto, pst_man_selec
      ...
      real apstd, aperture
      real smean, thresh, k_sigma_thresh
...
# ------------------------------------------------------------------------------    
  ...
  k_sigma_thresh = 3.5
  if (inter) {
    print ('\n Accept k_sigma_thresh value ('//k_sigma_thresh//')? (y/n)')
    print (' default = yes')
    check = yes
    # scan (check)
    if (check) { # Do nothing
      ...
      print ('\n Input new \'k_sigma_thresh\' value:')
      scan (k_sigma_thresh)
    }
  }
  ...
  if (sigma == 0.) {
    ...
    print (' threshold = k_sigma_thresh*sigma = '//k_sigma_thresh*sigma//' or else use')
    print (' \'SSTDEV\'= '//sstd//' value:')
    print (' threshold = k_sigma_thresh*STDDEV = '//k_sigma_thresh*sstd)
  ...
  findpars.threshold = k_sigma_thresh
  print (' Threshold = '//k_sigma_thresh*sigma)
  print ('Threshold = ', (k_sigma_thresh*sigma), >> (imname//'.parameters')) 
  ...
  if (inter) {
    ...
    print (' datapars.ccdread = "ENOISE"')
    #print (' datapars.gain = "GAIN"')
    print (' datapars.gain = "EGAIN"')
    print (' datapars.exposur = "EXPTIME"')
    #print (' datapars.filter = "FILTERS"')
    print (' datapars.filter = "FILTER"')
    check = yes
    # scan (check)
    if (check) {
      datapars.noise = "poisson"
      datapars.ccdread = "ENOISE" # "RDNOISE"
      datapars.gain = "EGAIN" # "GAIN"
      datapars.exposur = "EXPTIME"
      datapars.filter = "FILTER" # "FILTERS"              
    }
    else {
  ...
  # THIS WHILE CLOSES AFTER THE CHECKING OF THE
  ...
  while (psf_done == no) {
    ...
    while (star_search == no) {
      ...
      if (check) { 
        ...
        print (' Threshold value: ' // k_sigma_thresh)
        print (' ----------------------------------------------------- ')
        daofind.verif = no
        daofind.verb = yes
      ...
      print (' Threshold = '//k_sigma_thresh)
                ...
                if (var2==3) {
                  ...
                  if (var2==4) {
                    ...
                    if (check) {
                      ...
                      tvmark=no
                      thresh_change = yes
                      print ('\n Actual threshold = ' //k_sigma_thresh)
                      print (' Input new threshold value')
                      print ('\n (A higher threshold means less stars)')
                      scan (k_sigma_thresh)
                      findpars.threshold = k_sigma_thresh
                      print ('New Findpars.threshold = ', k_sigma_thresh, >> (imname//'.parameters'))
                      del (imname// '.coo.1')
                    }
                    else {
                      auxi=no
    ...
    #      }
      ...
      # The 'maxnpsf' number is the number of stars the 'pstselect'
      # task will look for. We set this number to 2% of the stars
      # found with MAG!=INDEF, always in the range [20, 100].
      print ('')
...
# This 'while' encloses the whole 'Core PSF photometry' (or 'Aperture photometry') structure
while (right_sustraction == no) {
  if (dopsf==yes) {
    print ('\n ----------------------------------------------------- ')
  ...
  while (first_psf == no) {
    ...
    if (do_pstselect) {
      ...
      while (pstselect_while) {
        ...
        print ('\n Let \'pstselect\' select '//maxnpsf//' stars automatically? (y/n)')
        print (' Else: select PSF stars manually')
        print (' default = yes')
        pstauto=yes
        # scan (pstauto)
        if (pstauto==no) {
        ...
        pstselect.mode = "h"
        pstselect.image = imname
        pstselect.photfile = imname//'.mag.1'
        pstselect.pstfile = imname//'.pstselect'
        ...
        # Output file is 'imname.pstselect'
        # pstselect ((imname), (imname//'.mag.1'), (imname//'.pstselect'))
        pstselect
        ...
        print ('\n Select manually from list of PSF candidates (else accept all)? (y/n)')
        print (' default = no')
        pst_man_selec=no
        scan (pst_man_selec)
        if (pst_man_selec==yes) {
          ...
          print (' r       radial plot')
          print (' c       contour plot')
          print (' m       histogram plot')
        ...
        # scan (check)
        if (check) {
            pstselect_while = no # This exits the 'pstselect' 'while'
        }
        else {
          delete (imname//'.pstselect')                              
          print ('\n Input new \'psfrad\'')
      ...
      }  # This bracket closes the 'pstselect_while' 'while'
  ...
  # ------------------------------------------------------------------------------
    ...
    while (core_psf==yes) {
      ...
      if (n == 0) {
        # This means 0 stars were selected as PSF and so,
        # the PSF selection must be performed again.
        nopsfstar = yes
      }
      else {
        nopsfstar = no   
      ...
      if (nopsfstar == no) {
        ...
        while (first_nstarsubstar == no) {
          ...
          print ('\n ------------------------------------------------------------')
          print ('\n Were the PSFs and their neighbors substracted cleanly? (y/n)')
  ...
  if ((m-42)/2 == (n-24)) {  # If this is true, that means there are NO PSF neighbour stars.
    ...
    copy ((imname) // '.fits', (imname) // '.sub.2.fits')
  }
  else {
    no_neighbours = no
    print ('\n ----------------------------------------------------- ')
  ...
  if (no_neighbours==no) {
    ...
    #   scan (check)
    #   if (check) {
    # Due to added noise by subtraction, we reset 'datamin'
  ...
  if (dopsf==yes) { # This means the PSF photometry will be made (ie: 'allstar' task will be used).
    ...
    print ('\n --------------------------------', >> (imname//'.parameters'))
    print ('\'Allstar\' task', >> (imname//'.parameters'))
    #print ('\n Daopars.sannulu = '//sannulu)
    #if (inter) {
    ...
    file_var = (imname//'.als.1')
    mr=0
    while (fscan (file_var,line) != EOF) {
      mr = mr + 1
    }
    print ('\n Allstar stars found = '//(mr-44)/2)
    print ('Allstar stars found = ', (mr-44)/2, >> (imname//'.parameters'))
    print ('')
    print ('')
    print ('\n -----------------------------------------------------------')
    print (' Display task                                               ')
...
if (doapert == yes) {
  ...
  if (l == 1) {
    print ('Aperture correction on als.1 file:' //avg)
    pcalc.mode = "h"
